...
def dynamic_time_warp(similarity_matrix):
    ...
    rows, cols = numpy.shape(localcost)

    def acc_cost(local_cost):
        accumulated_cost = numpy.zeros((len(rows), len(cols)))
        accumulated_cost[0.0, 0.0] = similarity_matrix[0, 0]
        for i in range(1, len(cols)):
            accumulated_cost[0, i] = similarity_matrix[0, i] + accumulated_cost[0, i - 1]
        ...
        for i in range(1, len(rows)):
            for j in range(1, len(cols)):
                accumulated_cost[i, j] = min(accumulated_cost[i - 1, j - 1], accumulated_cost[i - 1, j],
                                             accumulated_cost[i, j - 1]) + similarity_matrix[i, j]

        return accumulated_cost

    # BACKTRACING TO FIND THE SHORTEST (MIN) PATH THROUGH MATRIX
    def min_path(x, y, acc_cost1):
        path = [[len(x) - 1, len(y) - 1]]
        acc_1 = acc_cost(acc_cost1)
        cost = 0
        i = len(y) - 1
        j = len(x) - 1
        while i > 0 and j > 0:
            if i == 0:
                ...
                if acc_1[i - 1, j] == min(acc_1[i - 1, j - 1], acc_1[i - 1, j], acc_1[i, j - 1]):
                    i -= 1
                elif acc_1[i, j - 1] == min(acc_1[i - 1, j - 1], acc_1[i - 1, j], acc_1[i, j - 1]):
                    j -= 1
                else:
                    i -= 1
                    j -= 1
        ...
        path.append([0, 0])
        # for [y, x] in path:
        #     cost = cost + sim_matrix[x, y]
        return j[1:], i[1:]

    p, q = min_path(rows, cols, localcost)
    return p, q
...
def compare_two_images():
    ...
    print("original image similarity:") + str(image_similarity(source_image, target_image))
